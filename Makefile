...
solo-down: #@ Stop Thor solo
	docker compose -f apps/contracts/docker-compose.yaml down
solo-clean: #@ Clean Thor solo
	docker compose -f apps/contracts/docker-compose.yaml down -v --remove-orphans
...
NAV_CONTRACTS=cd apps/contracts
NETWORK ?= vechain_solo

# Contracts
contracts-compile: #@ Compile the contracts.
	$(NAV_CONTRACTS); yarn compile --network=$(NETWORK)
contracts-deploy: contracts-compile solo-up #@ Deploy the contracts to specified network (default: vechain_solo).
	$(NAV_CONTRACTS); npx hardhat run scripts/deploy.ts --network=$(NETWORK)
